...
def get_serial_ports():
    ...
    return ports if ports else ["/dev/tty.usbserial-110"]
...
def run_preprocessing(raw_signal, demographics: dict):
    ...
    Returns dict with intermediate signals + final prediction.
    """
    ...
    # 2. Bandpass filter ──────────────────────────────────────────────────
    # valleys, peaks, filtered = detect_valleys_ppg(
    #     resampled, fs=NEW_FS, low=0.5, high=6.0,
    # )
    ...
    # filtered = scaler_mm.fit_transform(filtered.reshape(-1, 1)).flatten()

    # 3. Valley detection + baseline removal ─────────────────────────────────────────────────
    ...
    # Fit a placeholder scaler (in production load the saved scaler.pkl)
    # Try to load the real scaler if present
    ...
    if os.path.exists(scaler_path):
        ...
        # Fallback: use approximate z-score normalisation
    ...
    if os.path.exists(model_path):
        try:
            ...
            # Demo: return a plausible synthetic prediction for testing
            prediction = float(np.random.normal(loc=100, scale=15))
    else:
        # No model file — return a demo value so UI can be verified
        prediction = float(np.random.normal(loc=100, scale=15))
...
with st.sidebar:
    ...
    duration = st.slider("Recording Duration (s)", min_value=10, max_value=60, value=20, step=5)
...
if st.session_state.status == "done" and st.session_state.preprocessed:
    ...
    st.markdown(
        ...
        <div class="glucose-banner">
          <div class="glucose-label">ESTIMATED BLOOD GLUCOSE</div>
            <div style="display:flex; flex-direction: row; justify-content:center; align-items-baseline; gap:0.5rem;">
                <div class="glucose-number">{pred:.1f}</div>
                <div class="glucose-label">mg/dL</div>
                <div class="glucose-number"">{mmol:.1f}</div>
                <div class="glucose-label"">mmol/L</div>
            </div>
